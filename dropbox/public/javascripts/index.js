$(function(){
	...
	$(function(e){
		...
		$.post('/json/setupKuaiPan', $this.serialize(), function(res){})
		//$.post('/json/setVerifier', $this.serialize(), function(res){})
		$.post('/json/setupApp', $this.serialize(), function(res){
			if (res.status == 200){
				$('.app-setup button.btn').popover('hide');
				$('.access').removeClass('hide');
				$('.appAccessLink').attr('href', res.request_token.authorize_url);
				//$('.access_token button.btn').attr('href', res.request_token.authorize_url);
			} else {
